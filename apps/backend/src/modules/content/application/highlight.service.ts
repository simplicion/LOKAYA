    ...
    return {
      ...
      stories: highlight.items.map(item => ({
        ...
        product: item.story.product ? {
          ...
          price: String(item.story.product.sellingPrice),
          originalPrice: item.story.product.mrp && item.story.product.mrp > item.story.product.sellingPrice ? String(item.story.product.mrp) : undefined,
        } : null,
        createdAt: item.story.createdAt,
