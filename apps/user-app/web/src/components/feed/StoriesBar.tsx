...
export function StoriesBar({ fallbackStories = [] }: StoriesBarProps) {
  ...
  // Use live feed from backend, or fallback to mock if completely empty
  const feedGroups: StoryViewerStoreGroup[] = (serverFeed && serverFeed.length > 0)
    ? serverFeed
    : fallbackStories.length > 0
      ? fallbackStories.map((s, idx) => ({
          storeId: s.id || `mock-${idx}`,
          storeName: s.label || 'Store',
          storeAvatar: s.imageUrl || 'https://i.pravatar.cc/150?img=1',
          isVerified: true,
